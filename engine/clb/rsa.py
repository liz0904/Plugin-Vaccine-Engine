...
def generate_prime(gen_bit):
    while 1:
        p = get_odd(gen_bit)  # 홀수를 만든다.
        if simple_rsa(p) == 1:  # 소수일 가능성 체크한다.
            return p

# n보다 작고, n과 서로소인 정수 e 찾기
# 또한 확장 유클리드 호제법을 이용해서 d * e / n으로 나눴을때 나머지가 1인 정수 d 찾기
# 인자값 : n - 정수
# 리턴값 : e, d
def get_integer(n):
    while 1:
...
#         pr_fname - 개인키 파일 이름
# 리턴값 : 키 생성 성공 여부
def create_key(public_fname='key.prk', private_fname='key.skr', debug=False):
    ...
    try:
        # 공개키와 개인키를 파일로 만들기
        open(public_fname, 'wt').write(pu_data)
        open(private_fname, 'wt').write(pr_data)
    except IOError:
        return False

    # 공개키와 개인키 생성 완료
    if debug:
        print('[CLOUD BREAD Notification] Make key SUCCESS : %s, %s' % (public_fname, private_fname))
    return True
